...
var navLightThemeColor = 'rgb(199, 76, 193)'
...
function createSection(objectContainer, specificSectionNum) {
  ...
  objectContainer.commentOnSection.forEach(Comment => {
    ...
    nameOfSection.innerHTML = `Section ${i + 1}`
    landingContainer.classList.add('landing__container')
    commentOnSection.setAttribute('style', "font-family: 'Harmattan', sans-serif;")
    img.setAttribute('src', objectContainer.imgForSections[i + 1])
    console.log(objectContainer.imgForSections[i + 1])
...
if (localStorage.getItem('name') == 'meray' && localStorage.getItem('password') == 'mmm') {
  form.setAttribute('style', 'display:none;')
  smoothScrolling()
  createSlideShow(objectContainer)
  showSlides(slideIndex)
  createHeader(objectContainer)
  createSection(objectContainer, 6)
...
function lightMode() {
  ...
  document.body.style.background = 'linear-gradient(0deg, rgb(231, 231, 231) 0%, rgb(204, 114, 182) 100%)';
...
imgs.forEach(img => {
  ...
  if (img.getBoundingClientRect().width < 800) {
    if (s % 2 == 1) {
